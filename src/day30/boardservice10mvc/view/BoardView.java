...
public class BoardView {
    ...
    public void mainPage(){ // view 시작 함수
        while (true){
            System.out.print("1.게시물등록 2.게시물출력 : 3. 게시물 삭제 : ");
    ...
    void delete(){
        ...
        System.out.println(" 삭제 할 번호는 ? : ");

        int choose = scan.nextInt();

        ArrayList<BoardDto> result2 = BoardController.getInstance().delete(choose);

        for( int index = 0 ; index<= result2.size()-1 ; index++ ){
            System.out.print("게시내용 : " + result.get( index ).getContent() );
            System.out.println("작성자 : " + result.get( index ).getWriter() );
            System.out.println("게시물 번호 : " + result.get(index).getNum() );
        }
