...
public class FileExample {
    public static void main(String[] args) {
        ...
        try {    // 예외가 발생 할 것 같은 코드 try{} 안에서 작ㄷ성한다.

            // c드라이브에 ' java ' 폴더 생성한다.
            //FileOutputStream fileOutput = new FileOutputStream("c:/java/test1.txt");
            FileOutputStream fileOutput = new FileOutputStream("c:./src/day25/test1.txt");
            // - 현재 프로젝트내 'src -> day25 -> test1.txt ' 파일을 생성한다
            fileOutput.write("Hello Java".getBytes());
            // fileOutput(객체).write[메소드]( 인자값1 );
        } catch (FileNotFoundException e) {
            // [특정] 예외가 발생했을때 실행되는 코드 작성한다.
            e.printStackTrace();
        } catch (Exception e) {
